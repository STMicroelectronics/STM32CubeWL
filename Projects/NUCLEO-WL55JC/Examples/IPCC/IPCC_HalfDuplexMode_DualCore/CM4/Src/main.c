...
/**
  ******************************************************************************
  * @file    IPCC/IPCC_HalfDuplexMode_DualCore/CM4/Src/main.c
...
int main(void)
{
  ...
  uint32_t index_cm4 = 0; /* A counter to execute several times the communication process */
  uint32_t index_cm0plus = 0xDEADDEAD; /* The index given by cm0plus core */
  ...
  *cpu2InitDone = CPU2_NOT_INITIALISED;
  ...
  err = HAL_IPCC_ActivateNotification(&hipcc, ChannelId, IPCC_CHANNEL_DIR_TX, HalfDuplex_receive_callback);
  if(HAL_OK != err)
  {
    Error_Handler();
  }
  ...
  while (index_cm4 <= 1000)
  {
    ...
    snprintf((char *)sharedBuffer, BUFFER_SIZE, "cpu1->cpu2:%04lu", (long unsigned int) index_cm4++);
    ...
    /* Wait until the callback has been triggered (On reception of a transaction) */
    /* At this step, note that HalfDuplex_receive is prefixed with "__IO" attribute */
    while(HalfDuplex_receive == 0);
    
    /* Reset HalfDuplex_receive value */
    HalfDuplex_receive = 0;
    
    /* Retrieve communication content and store in inside index_cm0plus */
    if( 1 != sscanf((char const *)sharedBuffer,"cpu2->cpu1:%04lu", (long unsigned int *) &index_cm0plus))
    {
      Error_Handler();
    }
  }
