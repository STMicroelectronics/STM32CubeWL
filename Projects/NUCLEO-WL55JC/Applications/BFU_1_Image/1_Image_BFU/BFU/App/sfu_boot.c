...
static SFU_ErrorStatus SFU_BOOT_SM_Run(void)
{
  ...
  while (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (SFU_BOOT_SecuritySafetyCheck() == SFU_SUCCESS)
    {
      ...
      {
        (void) SFU_BOOT_SetLastExecStatus(EXEC_ID_SECURE_BOOT, (uint8_t)m_StateMachineContext.CurrState);
      }
  ...
  (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_UNKNOWN);
...
static void SFU_BOOT_SM_CheckStatusOnReset(void)
{
  ...
  if (SE_INFO_ReadBootInfo(&e_se_status, &x_boot_info) == SE_SUCCESS)
  {
    ...
    if (SFU_BOOT_SetLastExecStatus(EXEC_ID_SECURE_BOOT, (uint8_t)m_StateMachineContext.CurrState) == SFU_SUCCESS)
    {
...
static void SFU_BOOT_SM_CheckNewFwToDownload(void)
{
  ...
  if ((*(uint32_t *)LOADER_COM_REGION_RAM_START) == STANDALONE_LOADER_BYPASS_REQ)
  {
...
#if (SECBOOT_LOADER == SECBOOT_USE_STANDALONE_LOADER)
  if ((*(uint32_t *)LOADER_COM_REGION_RAM_START) == STANDALONE_LOADER_INSTALL_REQ)
  {
...
static void SFU_BOOT_SM_DownloadNewUserFw(void)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    active_slot = SFU_IMG_GetFwImageSlot(&fw_image_header_validated);
    if (SFU_IMG_Validation(active_slot, &fw_image_header_validated) != SFU_SUCCESS)
    {
      /* CleanUp downloaded image (header preserved, for anti-rollback) */
      (void) SFU_IMG_InvalidateCurrentFirmware(active_slot);
  ...
  else
  {
    ...
    switch (e_ret_status_app)
    {
      case SFU_LOADER_ERR_COM:
        (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_COM_ERROR);
        break;
      case SFU_LOADER_ERR_FW_VERSION:
        (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_INCORRECT_VERSION);
        break;
      case SFU_LOADER_ERR_FW_LENGTH:
        (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FW_TOO_BIG);
        break;
      case SFU_LOADER_ERR_AUTH_FAILED:
        (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_HEADER_AUTH_FAILED);
        break;
...
static void SFU_BOOT_SM_ExecuteUserFw(void)
{
   ...
   */
  e_ret_status = SFU_BOOT_SetLastExecStatus(EXEC_ID_USER_APP, (uint8_t)m_StateMachineContext.CurrState);
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    else
    {
      ...
      (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_LOCK_SE_SERVICES_ERR);
  ...
  {
    TRACE("\r\n= [FWIMG] EXTERNAL FLASH CONFIGURATION FAILURE!");
...
static SFU_ErrorStatus SFU_BOOT_LaunchStandaloneLoader(void)
{
  ...
  if (SE_LockRestrictServices(&e_se_status) == SE_SUCCESS)
  {
    /* Give execution rights to standalone loader */
    e_ret_status = SFU_LL_SECU_SetProtectionMPU_StandaloneLoader();
  }
...
static SFU_ErrorStatus SFU_BOOT_SetLastExecStatus(uint8_t uExecID, uint8_t uLastExecState)
{
  ...
  if ((IS_VALID_EXEC_ID(uExecID)) && (IS_SFU_SM_STATE(uLastExecState)))
  {
    if (SE_INFO_ReadBootInfo(&e_se_status, &x_boot_info) == SE_SUCCESS)
    {
      ...
      x_boot_info.LastExecStatus = RESERVED_VALUE
                                   | ((uint32_t)uLastExecState << 8U)
                                   | ((uint32_t)uFwImageIdx << 16U)
                                   | ((uint32_t)uExecID << 24U);
...
static SFU_ErrorStatus  SFU_BOOT_ManageLastExecStatus(uint32_t uLastExecStatus)
{
  ...
  SE_StatusTypeDef         e_se_status;
  SE_BootInfoTypeDef       x_boot_info;
  SFU_EXCPT_IdTypeDef      u_last_error;
  ...
  if (SE_INFO_ReadBootInfo(&e_se_status, &x_boot_info) == SE_SUCCESS)
  {
...
#else
    UNUSED(uLastExecStatus);
#endif /* SFU_VERBOSE_DEBUG_MODE */

    /* Show the last execution error */
    u_last_error = (SFU_EXCPT_IdTypeDef) x_boot_info.LastExecError;
    TRACE("\r\n\t  INFO: Last execution detected error was: ");
    if (IS_SFU_EXCPT(u_last_error))
    {
      TRACE((char const *)m_aErrorStrings[u_last_error]);
    }
    else
    {
      TRACE("Unknown error.");
     ...
     */
    x_boot_info.LastExecError = (uint32_t) SFU_EXCPT_NONE;
    ...
    case SFU_RESET_OB_LOADER:
      ...
      */
      /* In the current implementation this event is not considered as an error to manage,
         ...
         But this is strictly related to the final system. If needed to be managed as an error
         please add the right error code in the following src code line.
         Typically we may implement a more clever check where we determine if this OB reset occurred only once because
         SB_SFU had to tune the OB initially, or if it occurred again after these initial settings. If so, we could
