    ...
    if (fw_verified_total_size <= fw_size)
    {
      ...
      if ((se_ret_status == SE_SUCCESS) && (*pSeStatus == SE_OK) && (fw_tag_len == SE_TAG_LEN))
      {
        ...
        else
        {
          ...
          if ((SlotNumber >= SLOT_ACTIVE_1) && (SlotNumber < SLOT_DWL_1))
          {
            (void) memcpy(&fw_tag_validated[SlotNumber - SLOT_ACTIVE_1][0U], fw_tag, SE_TAG_LEN);
          }
        }
      }
      else
...
SFU_ErrorStatus VerifySlot(uint8_t *pSlotBegin, uint32_t uSlotSize, uint32_t uFwSize)
{
   ...
   */
  if ((uFwSize % (uint32_t)sizeof(SFU_LL_FLASH_write_t)) != 0U)
  {
    fw_size = uFwSize + ((uint32_t)sizeof(SFU_LL_FLASH_write_t) - (uFwSize % (uint32_t)sizeof(SFU_LL_FLASH_write_t)));
  }
  else
  {
    fw_size = uFwSize;
  ...
  (void) memset((void *)&pattern1, 0xFF, sizeof(SFU_LL_FLASH_write_t));
  (void) memset((void *)&pattern2, 0x00, sizeof(SFU_LL_FLASH_write_t));

  /* Clean-up Check operation */
  slot_begin = (uint32_t) pSlotBegin;
  for (pdata = slot_begin + SFU_IMG_IMAGE_OFFSET + fw_size; 
      (pdata < (slot_begin + uSlotSize)) && (e_ret_status == SFU_SUCCESS);
       pdata += sizeof(SFU_LL_FLASH_write_t))
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      if ((memcmp((void *) &value, (void *) &pattern1, sizeof(SFU_LL_FLASH_write_t)) != 0) &&
          (memcmp((void *) &value, (void *) &pattern2, sizeof(SFU_LL_FLASH_write_t)) != 0))
      {
...
SFU_IMG_InitStatusTypeDef SFU_IMG_InitImageHandling(void)
{
...
#if  !defined(SFU_NO_SWAP)
  SFU_IMG_InitStatusTypeDef e_swap_ret_status;
#endif /*(GENERATOR_2_FW_IMAGE) && !(SFU_NO_SWAP) */
  ...
  for (i = 0U; i < SFU_NB_MAX_DWL_AREA; i++)
  {
...
#endif /* (__GNUC__) */
    {
      if(!((((SlotStartAdd[SLOT_DWL_1 + i]   - FLASH_BASE) / FLASH_PAGE_SIZE) > SFU_PROTECT_WRP_PAGE_END_1 )
        || (((SlotEndAdd[SLOT_DWL_1 + i] + 1 - FLASH_BASE) / FLASH_PAGE_SIZE) < SFU_PROTECT_WRP_PAGE_START_1)))
      {
  ...
  {
    e_ret_status = e_swap_ret_status;
  }
...
/**
  ...
  */
SFU_ErrorStatus SFU_IMG_CheckFwVersion(uint16_t CurrentVersion, uint16_t CandidateVersion)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;


  /* If the headre of the active firmware is not valid, the CurrentVersion is 0 */
...
SFU_ErrorStatus SFU_IMG_CheckCandidateVersion(uint32_t DwlSlot)
{
  ...
  curVer = SFU_IMG_GetActiveFwVersion(active_slot);

  if (SFU_IMG_CheckFwVersion(curVer, fw_image_header_to_test.FwVersion) != SFU_SUCCESS)
  {
  ...
  else
  {
    ...
    e_ret_status = SFU_IMG_CheckFwVersion(curVer, fw_image_header_to_test.FwVersion);
  }
