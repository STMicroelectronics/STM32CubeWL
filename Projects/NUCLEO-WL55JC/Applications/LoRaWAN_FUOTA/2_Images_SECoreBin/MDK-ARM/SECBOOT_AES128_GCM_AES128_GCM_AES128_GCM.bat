...
IF "%fwid%"=="2" (
set "priorbin=%projectdir%\\..\\Binary\\SBSFU_LoRaWAN_End_Node_DualCore_CM0PLUS.bin"
...
set "command=%python%%prepareimage% pack -m %magic% -k  %oemkey%  -r 112 -v %version% -n %nonce% -f %sfu% -t %sign% %sfb% -o 512 >> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set "command=%python%%prepareimage% header -m %magic% -k  %oemkey%  -r 112 -v %version% -n %nonce% -f %sfu% -t %sign% -o 512 %headerbin%  >> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
...
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
set /P header=<%projectdir%\header.txt >> %projectdir%\output_%fwid%.txt 2>>&1
...
set "command=%python%%prepareimage% diff -1 %ref_userapp% -2 %bin% %partialbin% -a 16 --poffset %partialoffset% >> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% enc -k %oemkey% -n %nonce% %partialbin% %partialsfu%  >> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% sign -k %oemkey% -n %nonce% %partialbin% %partialsign% >> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error

set "command=%python%%prepareimage% pack -m %magic% -k %oemkey%  -r 112 -v %version% -n %nonce% -f %sfu% -t %sign% -o 512 --pfw %partialsfu% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
