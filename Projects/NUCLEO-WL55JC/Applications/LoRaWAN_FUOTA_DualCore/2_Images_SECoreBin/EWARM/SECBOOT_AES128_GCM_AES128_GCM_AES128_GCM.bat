...
set "command=%python%%prepareimage% pack -m %magic% -k %oemkey%  -r 112 -v %version% -n %nonce% -f %sfu% -t %sign% -o %offset% --pfw %partialsfu% --ptag %partialsign% --poffset  %partialoffset% %partialsfb%>> %projectdir%\output_%fwid%.txt 2>&1"
%command%
IF %ERRORLEVEL% NEQ 0 goto :error
