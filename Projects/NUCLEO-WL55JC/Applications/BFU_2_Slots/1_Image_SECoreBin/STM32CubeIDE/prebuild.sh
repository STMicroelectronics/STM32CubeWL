...
if [ $ret -eq 0 ]; then
  kms_blob_ecckey=$SBSFUBootLoader"/1_Image_KMS_Blob/Binary/ECCKEY.txt"
  command=$cmd" "$prepareimage" inject -k "$kms_blob_ecckey" -f "$1"/../Inc/kms_platf_objects_config.h.pattern -p @ECDSA_BLOB_KEY@ "$kms_tmp
  echo $command
  $command
  ret=$?
  if [ $ret -eq 0 ]; then
    kms_blob_oemkey=$SBSFUBootLoader"/1_Image_KMS_Blob/Binary/OEM_KEY_COMPANY1_key_AES_CBC.bin"
