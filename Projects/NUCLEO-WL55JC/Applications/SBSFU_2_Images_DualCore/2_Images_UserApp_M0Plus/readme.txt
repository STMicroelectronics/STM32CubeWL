...
This application demonstrates firmware download capabilities and provides a set
of functions to test the active protections offered by Secure Boot and Secure Engine.
...
A terminal connected with the board via VCOM is needed to communicate with the
board and to select which feature to demonstrate.
...
@par Directory contents
   ...
   - 2_Images_UserApp_M0Plus/Src/sfu_app_new_image.c      Manage the new firmware image storage and installation
   ...
   - 2_Images_UserApp_M0Plus/Inc/sfu_app_new_image.h      Header file for sfu_app_new_image.c
...
This postbuild script:
 ...
 - prepares the new portion of firmware image: part of firmware image which changed vs a reference firmware image.
   ...
   application into RefUserApp.bin (in folder 2_Images_UserApp_M0Plus/IDE).
   ...
   application.
   At post processing stage, the portion of code containing the UserAppId will be identified as a difference vs
   the reference binary file and a PartialUserApp.sfb will be generated in 2_Images_UserApp_M0Plus/Binary folder.
...
Note :  for Linux users Minicom can be used but to do so you need to compile the UserApp project with the MINICOM_YMODEM switch enabled (ymodem.h)
