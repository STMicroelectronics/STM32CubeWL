...
static uint8_t alignedBuffer[SFU_COM_YMODEM_PACKET_SIZE] __attribute__((aligned(4)));
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
        ...
        ((m_uDwlAreaAddress + rx_size) >= (m_uDwlAreaStart + SFU_IMG_IMAGE_OFFSET)))
    {
      ...
      if (SFU_LL_FLASH_Write(&x_flash_info, (uint8_t *)m_uDwlAreaAddress, p_data, uLength) == SFU_SUCCESS)
      {
        ...
        m_uDwlAreaAddress += uLength + (((SE_FwRawHeaderTypeDef *)(uint32_t)fw_header)->PartialFwOffset % SLOT_SIZE(SLOT_SWAP));
