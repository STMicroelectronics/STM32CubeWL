...
SFU_ErrorStatus SFU_LL_SECU_SetProtectionMPU(void)
{
  ...
  for (mpu_region_num = 0U; mpu_region_num < (sizeof(MpuAreas) / sizeof(SFU_MPU_InitTypeDef)); mpu_region_num++)
  {
    ...
    MPU_InitStruct.IsShareable          = MPU_ACCESS_NOT_SHAREABLE;
    MPU_InitStruct.IsCacheable          = MpuAreas[mpu_region_num].Bufferable;
    MPU_InitStruct.IsBufferable         = MpuAreas[mpu_region_num].Cacheable;
...
/**
  ...
  *         This SE function should never be called.
  ...
  * @retval SE_ErrorStatus SE_ERROR.
  */
