...
static const int8_t SWAPPED[sizeof(SFU_LL_FLASH_write_t)] __attribute__((aligned(4))) = {0, 0, 0, 0, 0, 0, 0, 0};
static const int8_t NOT_SWAPPED[sizeof(SFU_LL_FLASH_write_t)] __attribute__((aligned(4))) = {-1, -1, -1, -1, -1, -1,
                                                                                             -1, -1};
...
static SFU_ErrorStatus FirmwareToRollback(uint32_t SlotNumber, SE_FwStateTypeDef MasterSlotState)
{
  ...
  if (SFU_SUCCESS == SFU_IMG_DetectFW(SlotNumber))
  {
    ...
    if (e_ret_status_se == SE_SUCCESS)
    {
      ...
      TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%d state = %d", SlotNumber, image_state);
...
{
  (void) memcpy(pNewHeader->PrevHeaderFingerprint, pPrevHeader->HeaderSignature, sizeof(pNewHeader->PrevHeaderFingerprint));
}
                                          ...
                                          SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    (void) memcpy(&magic[MAGIC_LENGTH / 2U], (uint8_t *) (buffer + SE_FW_AUTH_LEN + (MAGIC_LENGTH / 2U)), MAGIC_LENGTH / 2U);
...
static SFU_ErrorStatus DecryptImageInDwlSlot(uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  if ((se_ret_status == SE_SUCCESS) && (e_se_status == SE_OK))
  {
           ...
           (e_se_status == SE_OK))
    {
      ...
      if (size != 0U)
      {
        se_ret_status = SE_Decrypt_Append(&e_se_status, (uint8_t *)fw_encrypted_chunk, (int32_t)size, fw_decrypted_chunk,
      ...
      if ((se_ret_status == SE_SUCCESS) && (e_se_status == SE_OK) && (fw_decrypted_chunk_size == size))
      {
        ...
        if (e_ret_status == SFU_SUCCESS)
        {
          ...
          e_ret_status = SFU_LL_FLASH_Write(&flash_if_status, (uint8_t *)fw_dest_address_write,  fw_decrypted_chunk, size);
          STATUS_FWIMG(e_ret_status == SFU_ERROR, SFU_IMG_FLASH_WRITE_FAILED);
...
/**
  ...
  * @retval SFU_SUCCESS if successful, a SFU_ErrorStatus error otherwise.
  */
static SFU_ErrorStatus  CheckTrailerValid(uint32_t ActiveSlot, uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pValidHeader,
                                          SE_FwRawHeaderTypeDef *pTestHeader)
{
  SFU_ErrorStatus e_ret_status = SFU_ERROR;
  uint8_t fw_header_trailer_valid[SE_FW_HEADER_TOT_LEN];
  uint8_t fw_header_trailer_test[SE_FW_HEADER_TOT_LEN];
  uint8_t fw_header_active_slot[SE_FW_HEADER_TOT_LEN + VALID_SIZE];
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      if (memcmp(fw_header_active_slot, fw_header_trailer_valid, sizeof(fw_header_trailer_valid)) != 0)
      {
        /*
      ...
      else
      {
        ...
        e_ret_status = SFU_IMG_CheckFwVersion(pValidHeader->FwVersion, pTestHeader->FwVersion);
      }
    }
...
static SFU_ErrorStatus FirmwareToInstall(uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  uint8_t fw_header_dwl_slot[SE_FW_HEADER_TOT_LEN + VALID_SIZE]; /* header + VALID tags */
  uint32_t trailer_begin = (uint32_t) TRAILER_BEGIN(DwlSlot);
  uint32_t end_of_test_image;
  int32_t ret;
  /*
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      if ((trailer_begin < end_of_test_image) || (ret != 0))
      {
        ...
        if (1U == initialDeviceStatusCheck)
        {
          ...
          if (ret != 0)
          {
            TRACE("\r\n= [FWIMG] The headers in dwl slot and swap area do not match!");
...
static SFU_ErrorStatus PrepareCandidateImageForInstall(uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  if (e_ret_status != SFU_SUCCESS)
  {
    (void)SFU_BOOT_SetLastExecError((uint32_t) SFU_EXCPT_FLASH_ERROR);
#if defined(SFU_VERBOSE_DEBUG_MODE)
  ...
  if (e_ret_status != SFU_SUCCESS)
  {
    (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_DECRYPT_FAILURE);
#if defined(SFU_VERBOSE_DEBUG_MODE)
  ...
  if (e_ret_status != SFU_SUCCESS)
  {
    (void)SFU_BOOT_SetLastExecError((uint32_t) SFU_EXCPT_SIGNATURE_FAILURE);
#if defined(SFU_VERBOSE_DEBUG_MODE)
                                         ...
                                         SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  if (SFU_SUCCESS == e_ret_status)
  {
    ...
    if (e_ret_status != SFU_SUCCESS)
    {
      (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FWIMG_SWAP);
#if defined(SFU_VERBOSE_DEBUG_MODE)
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    {
      /* Memorize the error */
      (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FWIMG_MAGIC);
#if defined(SFU_VERBOSE_DEBUG_MODE)
      TRACE("\r\n= [FWIMG] Clean magic tag update failure!");
#endif /* SFU_VERBOSE_DEBUG_MODE */
    }
  }
...
SFU_IMG_InitStatusTypeDef SFU_IMG_CheckSwapImageHandling(void)
{
    ...
    if (SlotStartAdd[SLOT_DWL_1 + i] != 0U)
    {
      if (!(SFU_IMG_REGION_IS_MULTIPLE(SLOT_SIZE(SLOT_DWL_1 + i), SLOT_SIZE(SLOT_SWAP))))
      {
        TRACE("\r\n= [FWIMG] SLOT_DWL_%d size (%d) must be a multiple of swap size (%d)\r\n",
              SLOT_DWL_1 + i, SLOT_SIZE(SLOT_DWL_1 + i), SLOT_SIZE(SLOT_SWAP));
        e_ret_status = SFU_IMG_INIT_SLOTS_SIZE_ERROR;
      }
    }
  }
    ...
    if (SlotStartAdd[SLOT_DWL_1 + i] != 0U)
    {
      if (((int32_t)(SFU_IMG_CHUNK_SIZE - (TRAILER_INDEX(SLOT_DWL_1 + i) * sizeof(SFU_LL_FLASH_write_t)))) < 0)
      {
        ...
        TRACE("\r\n= [FWIMG] %d bytes required for the swap metadata of SLOT_DWL_%d is too much\r\n",
              (TRAILER_INDEX(SLOT_DWL_1 + i) * sizeof(SFU_LL_FLASH_write_t)), SLOT_DWL_1 + i);
  ...
  if(!((((SlotStartAdd[SLOT_SWAP]   - FLASH_BASE) / FLASH_PAGE_SIZE) > SFU_PROTECT_WRP_PAGE_END_1 )
    || (((SlotEndAdd[SLOT_SWAP] + 1 - FLASH_BASE) / FLASH_PAGE_SIZE) < SFU_PROTECT_WRP_PAGE_START_1)))
  {
  ...
  *pDwlSlotToInstall = SLOT_INACTIVE;
  ...
  for (i = 0U; i < SFU_NB_MAX_ACTIVE_IMAGE; i++)
  {
    ...
    if (SlotStartAdd[SLOT_ACTIVE_1 + i] != 0U)
    {
      ...
      {
        /* Slot configured ? */
        if (SlotStartAdd[SLOT_ACTIVE_1 + j] != 0U)
        {
          if (SFU_SUCCESS == CheckTrailerValid(SLOT_ACTIVE_1 + i, SLOT_DWL_1 + j, &fw_image_header_validated,
...
SFU_ErrorStatus SFU_IMG_TriggerImageInstallation(uint32_t DwlSlot)
{
   ...
   */
  if (fw_image_header_to_test.FwSize + SFU_IMG_IMAGE_OFFSET + TRAILER_SIZE(DwlSlot) <= SLOT_SIZE(active_slot))
  {
    e_ret_status = SFU_SUCCESS;
  }
...
SFU_ErrorStatus SFU_IMG_TriggerRollbackInstallation(uint32_t ActiveSlot, uint32_t DwlSlot)
{
  ...
  if (SFU_SUCCESS == e_ret_status)
  {
    ...
    if (0 == memcmp(active_header.PrevHeaderFingerprint, zero_buffer, sizeof(active_header.PrevHeaderFingerprint)))
    {
      e_ret_status = SFU_ERROR;
      (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FWIMG_ROLLBACK_PREV_ID);
#if defined(SFU_VERBOSE_DEBUG_MODE)
      TRACE("\r\n= [FWIMG] Rollback failure : backed-up FW not identified!");
#endif /* SFU_VERBOSE_DEBUG_MODE */
    }
  }
  ...
  if (SFU_SUCCESS == e_ret_status)
  {
    ...
    if (e_ret_status != SFU_SUCCESS)
    {
      (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FWIMG_ROLLBACK);
#if defined(SFU_VERBOSE_DEBUG_MODE)
      TRACE("\r\n= [FWIMG] Rollback process failure!");
#endif /* SFU_VERBOSE_DEBUG_MODE */
    }
  }
...
SFU_ErrorStatus SFU_IMG_UpdateImageState(uint32_t SlotNumber)
{
  ...
  if (e_ret_status_se == SE_SUCCESS)
  {
    if (image_state == FWIMG_STATE_NEW)
    {
      ...
      {
#if defined(SFU_VERBOSE_DEBUG_MODE)
        TRACE("\r\n= [FWIMG] SLOT_ACTIVE_%d state = %d failed to switch in FWIMG_STATE_SELFTEST", SlotNumber,
              image_state);
#endif /* (SFU_VERBOSE_DEBUG_MODE)     */
        e_ret_status = SFU_ERROR;
      }
