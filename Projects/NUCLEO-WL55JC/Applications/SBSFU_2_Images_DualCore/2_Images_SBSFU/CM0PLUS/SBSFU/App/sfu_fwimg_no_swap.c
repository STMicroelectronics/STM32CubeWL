                                          ...
                                          SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  if (e_ret_status == SFU_SUCCESS)
  {
    ...
    if (e_ret_status == SFU_SUCCESS)
    {
      ...
      if (memcmp(fw_header_active_slot, fw_header_trailer_test, sizeof(fw_header_trailer_test)) != 0)
      {
        /*
...
static SFU_ErrorStatus VerifyFwSignatureBeforeDecrypt(uint32_t DwlSlot, SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  uint8_t fw_authenticated_chunk[SFU_IMG_CHUNK_SIZE] __attribute__((aligned(4)));
           ...
           (fw_index < (pFwImageHeader->FwSize + SFU_IMG_IMAGE_OFFSET)))
    {
      ...
      if (e_ret_status == SFU_SUCCESS)
      {
        ...
        if (se_ret_status == SE_SUCCESS)
        {
          ...
          se_ret_status = SE_AuthenticateFW_Append(&e_se_status, fw_decrypted_chunk, (int32_t)size, fw_authenticated_chunk,
                                                   (int32_t *)&fw_authenticated_chunk_size);
#endif /* (SECBOOT_CRYPTO_SCHEME != SECBOOT_AES128_GCM_AES128_GCM_AES128_GCM) */
          fw_index += size;
        }
      }
                                                           ...
                                                           SE_FwRawHeaderTypeDef *pFwImageHeader)
{
  ...
  if (se_ret_status == SE_SUCCESS)
  {
     ...
     */
    e_ret_status = SFU_LL_FLASH_Erase_Size(&flash_if_status, (uint8_t *) SlotHeaderAdd[ActiveSlot], SFU_IMG_IMAGE_OFFSET);
    if (e_ret_status == SFU_SUCCESS)
    {
                                         ...
                                         SE_FwRawHeaderTypeDef *pTestHeader)
{
  ...
  if (e_ret_status != SFU_SUCCESS)
  {
    ...
    (void)SFU_BOOT_SetLastExecError((uint32_t)SFU_EXCPT_FLASH_ERROR);
#if defined(SFU_VERBOSE_DEBUG_MODE)
    TRACE("\r\n= [FWIMG] Dwl slot erasing failure!");
#endif /* SFU_VERBOSE_DEBUG_MODE */
    return e_ret_status;
  }
